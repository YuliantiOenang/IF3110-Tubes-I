...
<body>
	...
	<form id="registerform" method="post" action="registercard.php" name="myform">
		...
		if (form.cardnumber.value.length==0 || form.nama.value.length==0){	 
			...
			xmlhttp.onreadystatechange=function(){
				if (xmlhttp.readyState==4 && xmlhttp.status==200){
					...
					document.getElementById("cek").innerHTML='<input type="button" value="Ok" onclick="cekkartu(document.forms[\'myform\']);"> <a href="index.php">Skip</a>';
				}else{
			...
			xmlhttp.open("GET","registercard.php?cardnumber="+form.cardnumber.value+"&name="+form.nama.value,true);
			xmlhttp.send();
		}
	}
	</script>
