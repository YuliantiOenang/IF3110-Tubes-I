...
<head>
	...
	function AddToCart(IdBarang){
		...
		alert("AddToCart.php?action=add&id="+IdBarang+"&Jumlah="+Jumlah+"&Kategori=Daging");
		xmlhttp.open("GET","AddToCart.php?action=add&id="+IdBarang+"&Jumlah="+Jumlah+"&Kategori=Daging",true);
		xmlhttp.send();
	}
	</script>
</head>
<body>
	...
	<div id="container">
		...
		<div id="sort-bar" class="frame">
			...
			<button type="button" onClick="location.href='daging.php?SortBy=Nama'">Nama</button>
			<button type="button" onClick="location.href='daging.php?SortBy=Harga'">Harga</button>
		</div>
		
		<div id="dagangan">
			<!-- Dagangan Baris 1 -->
			<div class="frame">
			<?php $i=0;foreach($_SESSION['Daging'] as $Barang) { ?>
